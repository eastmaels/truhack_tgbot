...
const mongoURI = 'mongodb://localhost:27017/tweetsdb'; // Replace with your MongoDB URI
...
async function getNextTweet() {
    return await Tweet.findOne({ reviewed: false });
}
...
bot.onText(/\/start/, async (msg) => {
    ...
    const options = {
        reply_markup: {
            inline_keyboard: [
                [
                    { text: "Yes, it contains a candidate", callback_data: JSON.stringify({ id: tweet._id, candidate: true }) },
                    { text: "No, it doesn't", callback_data: JSON.stringify({ id: tweet._id, candidate: false }) }
                ]
            ]
        }
    };
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    bot.sendMessage(chatId, `Thanks! You classified this tweet as "${isCandidate ? 'Contains a candidate' : 'Does not contain a candidate'}".`);
    ...
    if (nextTweet) {
        const options = {
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: "Yes, it contains a candidate", callback_data: JSON.stringify({ id: nextTweet._id, candidate: true }) },
                        { text: "No, it doesn't", callback_data: JSON.stringify({ id: nextTweet._id, candidate: false }) }
                    ]
                ]
            }
        };
